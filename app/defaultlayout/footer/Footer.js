import React from 'react'

function Footer() {
    return (
        <footer>
            <h3 className='text-center'>OUR NEWSLETTER</h3>
            <div className="footer-section">
                <div className="container">
                    {/* <div className="sofa-img">
                    <img src="assets/images/97c387108399669.5fbd02d536a07-removebg-preview.png" alt="Image" className="img-fluid" />
                </div> */}

                    <div className="d-flex justify-content-center ">
                        <div className="">
                            <div className="subscription-form">
                                <h3 className="d-flex align-items-center"><span className="me-1"><i class="fa-solid fa-envelope"></i></span><span>Subscribe to Newsletter</span></h3>

                                <form action="#" className="row g-3">
                                    <div className="col-auto">
                                        <input type="text" className="form-control" placeholder="Enter your name" />
                                    </div>
                                    <div className="col-auto">
                                        <input type="email" className="form-control" placeholder="Enter your email" />
                                    </div>
                                    <div className="col-auto">
                                        <button className="btn btn-primary">
                                            <span className="fa fa-paper-plane"> S u b s c r i b e</span>
                                        </button>
                                    </div>
                                </form>

                            </div>
                        </div>
                    </div>

                    <div className="row g-5 mb-5">
                        <div className="col-lg-6 col-6 col-md-3">
                            <div className="mb-4 footer-logo-wrap"><a href="#" className="footer-logo">Desi Ghee<span>.</span></a></div>
                            <p className="mb-4">Donec facilisis quam ut purus rutrum lobortis. Donec vitae odio quis nisl dapibus
                                malesuada. Nullam ac aliquet velit. Aliquam vulputate velit imperdiet dolor tempor tristique.
                                Pellentesque habitant</p>

                            <ul className="list-unstyled custom-social">
                                <li><a href="#"><span className="fa fa-brands fa-facebook-f"></span></a></li>
                                <li><a href="#"><span className="fa fa-brands fa-twitter"></span></a></li>
                                <li><a href="#"><span className="fa fa-brands fa-instagram"></span></a></li>
                                <li><a href="#"><span className="fa fa-brands fa-linkedin"></span></a></li>
                            </ul>
                        </div>

                        <div className="col-lg-6 col-6 col-md-3">
                            <div className="row links-wrap">
                                <div className="col-6 col-sm-6 col-md-3">
                                    <ul className="list-unstyled">
                                        <li><a href="/page/about">About us</a></li>
                                        {/* <li><a href="#">Services</a></li> */}
                                        {/* <li><a href="#">Blog</a></li> */}
                                        <li><a href="/page/contact">Contact us</a></li>
                                    </ul>
                                </div>

                                {/* <div className="col-6 col-sm-6 col-md-3">
                                    <ul className="list-unstyled"> */}
                                {/* <li><a href="#">Support</a></li> */}
                                {/* <li><a href="#">Knowledge base</a></li> */}
                                {/* <li><a href="#">Live chat</a></li> */}
                                {/* </ul> */}
                                {/* </div> */}

                                <div className="col-6 col-sm-6 col-md-3">
                                    <ul className="list-unstyled">
                                        <li><a href="#">Jobs</a></li>
                                        <li><a href="#">Our team</a></li>
                                        <li><a href="#">Leadership</a></li>
                                        <li><a href="#">Privacy Policy</a></li>
                                    </ul>
                                </div>

                                <div className="col-6 col-sm-6 col-md-3">
                                    <ul className="list-unstyled">
                                        <li><a href="#">Nordic Chair</a></li>
                                        <li><a href="#">Kruzo Aero</a></li>
                                        <li><a href="#">Ergonomic Chair</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>

                    </div>

                    <div class="border-top copyright">
                        <div class="row pt-4">
                            <div class="col-lg-6">
                                <p class="text-center text-lg-start">Copyright ©
                                    2024. All Rights Reserved.
                                </p>
                            </div>

                            <div class="col-lg-6 text-center text-lg-end">
                                <ul class="list-unstyled d-inline-flex ms-auto">
                                    <li class="me-4"><a href="#">Terms &amp; Conditions</a></li>
                                    <li><a href="#">Privacy Policy</a></li>
                                </ul>
                            </div>

                        </div>
                    </div>

                </div>
            </div>
        </footer>
    )
}

export default Footer